...
| 361| **`wc` multiple files** — `wc a b c` counts several files (lines/words/bytes each, name-labeled) plus a `total` line. Completes multi-file behavior across the core text tools (cat/grep/wc). Verified: `wc HELLO.TXT MOTD.TXT` → per-file counts + correct total (3/23/120) | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
