...
| 364| **`grep -n` + combinable flags** — `-n` prefixes each match with its line number; flags now combine (`grep -in PAT`, a multi-char `-in` flag-token parse). Verified: `grep -n a MOTD.TXT` → `1:`/`2:` line-numbered matches; `grep -in HELLO MOTD.TXT` → case-insensitive match on line 2 | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
