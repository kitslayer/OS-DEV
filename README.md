...
| —  | **Committed fuzz-test infrastructure** (not a milestone — quality): `make check` runs **five** ASan/UBSan host-side regression/fuzz suites over the kernel's untrusted-input parsers — `jstest` (JS engine), `imgtest` (jpeg/png/gif/inflate, locks the M422 fix), `x509test` (cert DER), `nettest` (packet parser + reassembly), and `fstest` (the FAT32 read path over corrupt/cyclic on-disk structures — `#include`s fat32.c, stubs the disk, fuzzes 12k corrupt images; locks the M435 `cluster_in_range` guard, the chain cycle guard, and the dir-recursion depth caps). All five are verified to catch a reintroduced bug (e.g. removing fat32's dir-recursion depth caps trips an ASan stack-overflow). See [docs/422-untrusted-input-security-audit.md](docs/422-untrusted-input-security-audit.md) | [tests/](tests/) |
