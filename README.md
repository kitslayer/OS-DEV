...
| 283| **Position-handle WRITE** — `.textContent`/`.innerHTML`/`setAttribute`/`remove` on a selector match, splicing the page source + re-rendering. Splice **duplicated** (not refactored) from the id path → zero regression to the working browser; the offset is re-validated each use so a stale/forged offset fails closed (memory-safe; single-match writes exact, multi-write best-effort) | [kernel/browser.c](kernel/browser.c), [kernel/js.c](kernel/js.c) |
