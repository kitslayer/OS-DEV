...
| —  | **Committed fuzz-test infrastructure** (not a milestone — quality): `make check` runs four ASan/UBSan host-side regression/fuzz suites over the kernel's untrusted-input parsers — `jstest` (JS engine), `imgtest` (jpeg/png/gif/inflate, locks the M422 fix), `x509test` (cert DER), `nettest` (packet parser + reassembly). All four are verified to catch a reintroduced OOB. See [docs/422-untrusted-input-security-audit.md](docs/422-untrusted-input-security-audit.md) | [tests/](tests/) |
