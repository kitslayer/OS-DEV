...
| —  | **Committed test infrastructure** (not a milestone — quality): `make check` runs **six** ASan/UBSan host-side suites — five over the kernel's untrusted-input parsers: `jstest` (JS engine), `imgtest` (jpeg/png/gif/inflate, locks the M422 fix), `x509test` (cert DER), `nettest` (packet parser + reassembly), and `fstest` (the FAT32 driver — `#include`s fat32.c, stubs the disk: 12k corrupt-image iterations over the read path locking the M435 `cluster_in_range` guard + the chain cycle guard + the dir-recursion depth caps, plus 8k accumulating create/write/delete/mkdir ops confirming the write path is memory-safe) — plus `kattest`, a **known-answer test** of the from-scratch crypto vs published RFC/FIPS vectors (SHA-256/384/512, HMAC, HKDF, AES, AES-GCM, ChaCha20-Poly1305 incl. forged-tag rejection, X25519), locking the TLS 1.3 crypto foundation against silent regression. Each suite is verified to catch a reintroduced bug (e.g. removing fat32's depth caps trips an ASan stack-overflow; any crypto-output change fails its vector). Consolidated map of trust boundaries + audit results + test coverage: [docs/438-security-and-test-posture.md](docs/438-security-and-test-posture.md) | [tests/](tests/) |
