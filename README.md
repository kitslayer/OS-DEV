...
| 365| **shell `todo`** — a persistent to-do manager over `TODO.TXT`: `todo` lists items (numbered, `[x]`/`[ ]`), `todo add <text>` appends one, `todo done N` toggles item N's checkbox. A real productivity tool needing no menu slot. Verified: two adds → `todo done 1` → list shows `1. [x] buy milk` / `2. [ ] write code` | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
