...
| 349| **Simon** — a 24th app, an audio-visual memory game (a genre the suite lacked): a growing sequence of four coloured pads, each with its own PC-speaker tone, that you echo with keys 1-4 (one step longer each round), with a persistent best run (`SIMON.HI`). Caught+fixed a real input bug — a "press any key" game-over handler was catching a spurious key-up event and instantly masking the game-over screen; now SPACE restarts. Verified in-OS: flash → wrong guess → stable game-over | [user/simon.c](user/simon.c) | [user/typing.c](user/typing.c) |
