...
| 360| **`grep` across multiple files** — `grep PAT a b c` now searches several files, prefixing each match with its filename (single-file output unchanged: indented, no prefix). Verified: `grep a HELLO.TXT MOTD.TXT` → prefixed matches from both; `grep Hello HELLO.TXT` → unprefixed | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
