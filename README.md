...
| 359| **`cat` multiple files** — `cat a b c` now concatenates several files (its namesake behavior; was single-file only), splitting on spaces (safe for spaceless 8.3 names). Verified: `cat HELLO.TXT MOTD.TXT` prints both files' contents in order | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
