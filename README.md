...
| 363| **`grep -i`** — case-insensitive search (`grep -i hello` matches "Hello"), the most common grep option. Verified: `grep -i hello HELLO.TXT` matches, while plain `grep hello HELLO.TXT` reports no match (case-sensitivity preserved without the flag) | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
