...
| —  | **Committed fuzz-test infrastructure** (not a milestone — quality): `make check` runs **five** ASan/UBSan host-side regression/fuzz suites over the kernel's untrusted-input parsers — `jstest` (JS engine), `imgtest` (jpeg/png/gif/inflate, locks the M422 fix), `x509test` (cert DER), `nettest` (packet parser + reassembly), and `fstest` (the FAT32 driver — `#include`s fat32.c, stubs the disk: 12k corrupt-image iterations over the read path locking the M435 `cluster_in_range` guard + the chain cycle guard + the dir-recursion depth caps, plus 8k accumulating create/write/delete/mkdir ops confirming the write path — the kernel's most-fragile parser — is memory-safe). All five are verified to catch a reintroduced bug (e.g. removing fat32's dir-recursion depth caps trips an ASan stack-overflow). See [docs/422-untrusted-input-security-audit.md](docs/422-untrusted-input-security-audit.md) | [tests/](tests/) |
