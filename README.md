...
| 371| **shell `uniq`** — `uniq FILE` drops adjacent duplicate lines (the classic dedup, each line compared to the previous *printed* one). Verified: two identical `todo add dup` lines → `uniq TODO.TXT` shows one while `cat` shows both | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
