...
| 387| **Conway's Game of Life — a browser simulation** (`file:life.htm`, demo index) — a 24×12 grid seeded with `Math.random`, rendered as a `<pre>` through the DOM; the Step button advances one generation applying Conway's rules (2-D JS arrays + 8-neighbour counting), Reseed makes a new board. Verified live: the seeded grid rendered and Step advanced it to **Generation 1** with an evolved pattern | [tools/mkfatfs.c](tools/mkfatfs.c) |
