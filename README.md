...
| 350| **shell `apps` command** — lists every runnable program name, sourced from the kernel's `progs[]` table (single source of truth, so it never goes stale), to complement the F9 Apps menu with shell-side discovery; the `run` error message now points to it instead of a hardcoded 6-app subset. New `SYS_apps`/`app_list_names` (per-write bounds-guarded). Verified in-OS: lists all 24 apps | [kernel/app.c](kernel/app.c), [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
