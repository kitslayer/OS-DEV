...
| 368| **`grep -c`** — counts matching lines instead of printing them (a total across the given files; combinable, e.g. `-ic`), completing the common grep options (`-i`/`-n`/`-c`/`--`). Verified: `grep -c a MOTD.TXT` → 2, `grep -c xyz MOTD.TXT` → 0 | [user/shell.c](user/shell.c) | [user/typing.c](user/typing.c) |
