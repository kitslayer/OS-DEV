 ...
 * A bash-style preprocessing pass run after variable/alias expansion and before
 * glob: a{1,2}b -> "a1b a2b", {1..4} -> "1 2 3 4", {a..d} -> "a b c d". Safe +
 * additive: a `{...}` is only expanded when it has no spaces and either a
 * top-level comma or a valid range, and is not preceded by `$` -- so ${VAR},
 * $(...), function bodies `f() { ...; }` and command groups `{ cmd; }` (spaces,
 * or no comma/range) are left untouched. Multiple groups expand by repeated
 * single-group passes, giving the cartesian product ({a,b}{1,2} -> a1 a2 b1 b2). */
