/*
 * dlibc.c — the libc shim that lets the vendored doomgeneric sources build and
 * link against this OS's tiny userspace runtime (ulib + umalloc + syscalls).
 *
 * It implements the subset of <stdio.h> <stdlib.h> <string.h> <strings.h>
 * <ctype.h> <math.h> that the DOOM sources actually reference (verified by
 * grepping user/doom/*.c).  The malloc family and the mem* primitives are NOT
 * redefined here — they live in ulib.c/umalloc.c and are merely declared in the
 * shim headers.
 *
 * The interesting part is file I/O.  DOOM opens the WAD with fopen(path,"rb")
 * and then fseek/fread/ftell against it; we back a read-mode FILE by slurping
 * the whole file into a malloc'd buffer up front (sys_readfile).  Write-mode
 * FILEs accumulate into a growable buffer flushed to sys_writefile on
 * fflush/fclose (savegames/config).
 */
...
void abort(void)       { print("doom: abort()\n"); sys_exit(134); for (;;) {} }

void __doom_assert_fail(const char *expr, const char *file, int line)
{
    char buf[256];
    snprintf(buf, sizeof buf, "doom: assertion failed: %s (%s:%d)\n", expr, file, line);
    print(buf);
    sys_exit(134);
    for (;;) {}
}
...
{
    size_t i = 0;
    /* strip any leading "./" the d_iwad path search might prepend */
    if (in[0] == '.' && (in[1] == '/' || in[1] == '\\')) in += 2;
    for (; in[i] && i + 1 < outmax; i++) {
        char c = in[i];
...
/* sscanf: only the conversions DOOM uses — %x %X %i %d %o with an optional
 * leading-whitespace skip in the format and a leading '0'/'0x' in the input. */
int sscanf(const char *str, const char *fmt, ...)
{
    ...
    for (const char *p = fmt; *p; p++) {
        if (*p == ' ') { while (isspace((unsigned char)*s)) s++; continue; }
        if (*p != '%') {
            if (*p == *s) { s++; continue; }
            else break;
        }
        ...
        if (conv == 'x' || conv == 'X' || conv == 'i' || conv == 'd' ||
            ...
            /* 'i' auto-detects 0x / 0 prefixes via strtol(base 0); but DOOM's
             * "%i" inputs are decimal here, so base 10 is fine, while still
             * letting strtol's base-0 logic kick in when prefixed. */
            char *end;
            ...
            *out = (int)v;
            ...
            while (*s && !isspace((unsigned char)*s) && (!havew || n < width))
                out[n++] = *s++;
            out[n] = '\0';
            if (n) assigned++; else break;
        } else if (conv == '[') {
            /* handle %[^\n] (read until newline) used by config loader */
            p++;
            int negate = 0;
            if (*p == '^') { negate = 1; p++; }
            char setbuf[8]; int sl = 0;
