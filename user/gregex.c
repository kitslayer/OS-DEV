 ...
 *
 * Keys: type to edit the active field · Tab cycles pattern -> flags -> subject ·
 *   Backspace deletes · Esc quits.  Launch: `gregex` or the Apps menu.
 */
#include "ulib.h"
...
static int  field;                         /* 0 = pattern, 1 = flags, 2 = subject */
...
static void edit_field(int k) {
    ...
    else { buf = subj; len = &subjlen; max = SUBJMAX; }
