...
print(document.querySelector("p").tagName, document.querySelectorAll(".item")[1].tagName);  // P LI (element.tagName, uppercased; M301)
...
print("-- Proxy get/set traps (M-proxy) --");
var _pg=new Proxy({a:1}, { get(t,k,r){ return (k in t)?t[k]:"DFLT"; } }); print(_pg.a, _pg.zzz, (_pg["a"]+","+_pg["q"]));  // 1 DFLT 1,DFLT (get trap: own key read-through, missing key -> default; works via . and [])
var _pgp=false; var _pg2=new Proxy({x:21}, { get(t,k,recv){ _pgp=(recv===_pg2); return t[k]*2; } }); print(_pg2.x, "recvIsProxy="+_pgp);  // 42 recvIsProxy=true (get trap transforms; 3rd arg is the proxy itself)
var _slog=[]; var _back={}; var _ps=new Proxy(_back, { set(t,k,v,r){ _slog.push(k+"="+v); if(v>=0) t[k]=v; return true; } }); _ps.s=5; _ps.bad=-1; print(_slog.join(","), "s="+_back.s, "bad="+_back.bad);  // s=5,bad=-1 s=5 bad=undefined (set trap records every write; handler validates -> only s>=0 reaches the target)
var _tl={n:7}; var _pt=new Proxy(_tl, {}); var _r0=_pt.n; _pt.n=99; _pt.m=3; print(_r0, _tl.n, _tl.m, _pt.m);  // 7 99 3 3 (trap-less proxy: reads AND writes pass straight through to the target)
var _pk=new Proxy({a:1,b:2,c:3}, {}); print(Object.keys(_pk).join(","), Object.values(_pk).join(","), JSON.stringify(_pk));  // a,b,c 1,2,3 {"a":1,"b":2,"c":3} (Object.keys/values + JSON.stringify resolve a trap-less proxy to its target)
var _pk2=new Proxy({a:1,b:2}, { get(){ return 9; } }); var _fk=""; for(var _fkk in _pk2) _fk+=_fkk; print(Object.keys(_pk2).join(","), _fk, typeof _pk2);  // a,b ab object (enumeration is target's OWN keys even WITH a get trap (no ownKeys trap leaks); for-in same; typeof proxy = object)
var _pin=new Proxy({a:1}, {}); var _pi=("a" in _pin)+","+("z" in _pin); delete _pin.a; print(_pi, ("a" in _pin));  // true,false false (in/delete on a proxy operate on the target: a present then deleted)
var _pm=new Proxy({greet(){ return "hi:"+this.who; }, who:"px"}, {}); print(_pm.greet());  // hi:px (method call through a proxy: method resolved via get trap, this = proxy)
