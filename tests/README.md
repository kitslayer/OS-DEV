...
| `svgtest`  | `kernel/svg.c` | The from-scratch integer-only SVG rasterizer (parses untrusted web XML in-kernel). 5 unit cases that must render correctly (rect, viewBox scaling, circle + cubic-bezier path, stroked polygon, named colors) plus ~520k fuzz iterations: 100k random bytes, 100k mutations of valid SVG, 320k structured (random shape/path/attr trees), and adversarial inputs (deep nesting, huge coordinate counts, truncation). Locks bounds-safety on the scanline-fill crossings buffer, the per-shape point list in caller scratch, and `parse_num` against the 3 UB bugs the author fuzz first caught (negative shift, `num<<16` int64 overflow). |
