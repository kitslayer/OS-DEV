...
| `fstest`  | `kernel/fat32.c` | A valid minimal FAT32 image then 12k corrupted copies (BPB/FAT/root-dir bytes randomized) through `fat32_mount`/`list`/`read`/`find`/`tree`. Locks the M435 `cluster_in_range` guard, the cluster-chain cycle guard, and the dir-recursion depth caps: a corrupt/cyclic FAT must never OOB or hang. `#include`s fat32.c and stubs the disk (`ata_read`/`ata_write` → an in-memory image) + `vfs_register`. |
