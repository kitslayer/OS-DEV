...
if echo "$out" | grep -q "Hello from OS-DEV"; then
    echo "PASS: in-guest httpd app served a page over the from-scratch TCP stack (host curl received it)"
    echo "  served: $(echo "$out" | grep -o '<h1>[^<]*</h1>' | head -1)"
    exit 0
else
