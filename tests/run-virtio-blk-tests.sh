#!/bin/sh
# In-guest assertion for the virtio-blk driver (kernel/virtio_blk.c). Boots the
# real kernel headless under QEMU with a SECOND disk attached over LEGACY virtio
# (-device virtio-blk-pci,disable-modern=on,disable-legacy=off), captures COM1,
# and asserts the driver brought the device up and READ ITS KNOWN CONTENT BACK:
#
#   - we build a tiny raw image whose every sector holds a recognizable marker
#     ("VIRTIO-BLK SECTOR <n>!") plus a deterministic byte pattern, and compute,
#     on the host, the additive checksum the kernel self-test logs per sector;
#   - the kernel's virtio_blk_selftest() reads sectors 0..2 off the virtio disk
#     and logs each sector's checksum + first 16 bytes;
...
#     with no fault — the virtio disk is purely additional.
#
# Exit 0 = pass. SKIPs cleanly if QEMU or python3 is absent. Companion to
# run-boot-tests.sh (no virtio disk -> the driver must cleanly no-op there).
set -e
cd "$(dirname "$0")/.."
...
if ! command -v "$QEMU" >/dev/null 2>&1; then
    echo "SKIP: virtio-blk test ($QEMU not found)"
    exit 0
fi
if ! command -v python3 >/dev/null 2>&1; then
    echo "SKIP: virtio-blk test (python3 not found)"
    exit 0
fi
...
for s in range(NSEC):
    off = s * SEC
    marker = ("VIRTIO-BLK SECTOR %d!" % s).encode()
...
echo "booting kernel headless under QEMU with a virtio-blk second disk (COM1 capture)..."
...
# The driver found + brought up the legacy virtio block device.
require "virtio-blk up"                       "virtio-blk device probed + virtqueue brought up"
...
require "VIRTIO-BLK SECTO"                     "on-disk marker string read back verbatim"
...
require "launching the desktop environment"   "reached desktop launch (no fault on the virtio path)"
...
if [ "$fail" -eq 0 ]; then
    ...
    echo "FAIL: in-guest virtio-blk test"
