...
static const struct {
    ...
    { "SEARCH  HTM", "<h1>Form submit (GET)</h1>"
        "<p>Type into the field (Tab/n to it, Enter to edit, type, Enter to finish), then follow the <b>[Submit]</b> button. The browser builds <code>action?name=value</code> from the form's named fields and navigates &mdash; a real HTML GET form submission.</p>"
        "<form action=\"https://example.com/\">"
        "<p>Your query: <input id=\"q\" name=\"q\" placeholder=\"type something here\"></p>"
        "<p>Category: <input id=\"cat\" name=\"cat\" value=\"general\"></p>"
        "<p><input type=\"submit\" value=\"Submit to example.com\"></p>"
        "</form>"
        "<p>After you submit, the address bar shows <code>https://example.com/?q=YOUR+TEXT&amp;cat=general</code> &mdash; spaces become <code>+</code>, specials are percent-encoded, and the default field value is included. example.com loads and ignores the query.</p>"
        "<p>Point <code>action</code> at a search engine and the very same form becomes a working web search box.</p>" },
