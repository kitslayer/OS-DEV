...
static const struct {
    ...
    { "FLEX    HTM", "<style>.row{display:flex}</style><h2>Flexbox (display:flex)</h2><p>Three items in a <b>display:flex</b> row &mdash; they should sit side by side on one line, spaced by <b>gap:40px</b>:</p><div style=\"display:flex;gap:40px\"><div>Apple</div><div>Banana</div><div>Cherry</div></div><p>The same three divs <b>without</b> flex (default block flow) stack vertically instead:</p><div><div>Apple</div><div>Banana</div><div>Cherry</div></div><p>And a flex row from a <b>&lt;style&gt; rule</b> <code>.row{display:flex}</code> (not inline):</p><div class=\"row\"><div>Red</div><div>Green</div><div>Blue</div></div><p>A <b>flex-direction:column</b> flex stacks vertically (not a row):</p><div style=\"display:flex;flex-direction:column\"><div>Top</div><div>Middle</div><div>Bottom</div></div><p><b>justify-content:center</b> &mdash; the row is centred in the content area:</p><div style=\"display:flex;justify-content:center\"><div>One</div><div>Two</div><div>Three</div></div><p><b>justify-content:flex-end</b> &mdash; the row is pushed to the right:</p><div style=\"display:flex;justify-content:flex-end\"><div>One</div><div>Two</div><div>Three</div></div><p>Back to normal flow after the flex container.</p>" },
