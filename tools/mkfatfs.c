...
static const struct {
    ...
    { "ITER    JS ", "var s=Symbol(\"tag\");var range={from:1,to:4,[Symbol.iterator](){var cur=this.from,end=this.to;return{next(){if(cur<=end)return{value:cur++,done:false};return{value:0,done:true};}};}};var sum=0;for(var v of range)sum+=v;print(\"typeof \"+(typeof s)+\" sum \"+sum);" },
