...
static const struct {
    ...
    { "ATTR    HTM", "<h1>Reading attributes</h1>"
        "<p>JavaScript can read any element's HTML attributes with <code>getAttribute</code> &mdash; the href of a link, custom <code>data-*</code> values, a field's type, and so on.</p>"
        ...
        "<p id=\"out\">(click to read the link's href and data-note)</p>"
        "<p>The button calls <code>getElementById('lnk').getAttribute('href')</code> and <code>getAttribute('data-note')</code>, pulling the values straight out of the page's HTML. A missing attribute returns <code>null</code>.</p>" },
